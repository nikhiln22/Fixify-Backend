import { RegisterResponseDTO, SignupUserDataDTO, tempUserResponseDTO, verifyOtpDataDTO } from "../interfaces/DTO/IServices/userService.dto";
...
export class UserService implements IuserService {
    ...
    constructor(userRepo: IuserRepository, emailService: EmailService, otpService: OTPService, passwordService: PasswordHasher) {
    ...
    async userSignUp(data: SignupUserDataDTO): Promise<tempUserResponseDTO> {
        try {
            ...
            if (result.success) {
                return { message: "user already exists", success: false, status: HTTP_STATUS.BAD_REQUEST }
            }
            ...
            return { message: "User created successfully,OTP sent", email, tempUserId: response.tempUserId.toString(), success: true, status: HTTP_STATUS.CREATED };
        } catch (error) {
    ...
    async verifyOtp(data: verifyOtpDataDTO): Promise<RegisterResponseDTO> {
        try {
            ...
            const tempUserResponse = await this.userRepository.findTempUserById(tempUserId);
            console.log("tempUser:", tempUserResponse);
            if (!tempUserResponse.success || !tempUserResponse.tempUserData) {
                return { success: false, message: "Temporary user not found or expired", status: HTTP_STATUS.NOT_FOUND };
            }
            const tempUser = tempUserResponse.tempUserData;
            if (tempUser.otp !== otp) {
                return { success: false, message: "Invalid OTP", status: HTTP_STATUS.UNAUTHORIZED };
            }
            const userData = {
